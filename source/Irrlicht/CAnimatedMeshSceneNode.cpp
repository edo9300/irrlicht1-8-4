	...
	else if (Looping)
	{
		...
		if (FramesPerSecond > 0.f) //forwards...
		{
			if (CurrentFrameNr > EndFrame)
				CurrentFrameNr = StartFrame + (f32)fmod(CurrentFrameNr - StartFrame, EndFrame-StartFrame);
		}
		else //backwards...
		{
			if (CurrentFrameNr < StartFrame)
				CurrentFrameNr = EndFrame - (f32)fmod(EndFrame - CurrentFrameNr, EndFrame-StartFrame);
		}
	}
	else
	{
