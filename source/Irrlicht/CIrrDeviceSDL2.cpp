...
static void fixModifiers() {
	if(modifiers) {
		...
		unsigned sdl_state = (unsigned)SDL_GetModState() & ~(KMOD_CAPS | KMOD_NUM | KMOD_SCROLL);
		if (mod & 0x8)
			sdl_state |= KMOD_CAPS;
		if (mod & 0x20)
			sdl_state |= KMOD_NUM;
		if (mod & 0x10)
			sdl_state |= KMOD_SCROLL;
		SDL_SetModState((SDL_Keymod)sdl_state);
	}
}
#endif
		...
		case SDL_KEYUP:
			{
				...
				if((key & (~UINT32_C(0x0F))) == 0x60) {
					if((SDL_event.key.keysym.mod & KMOD_NUM) == 0) {
...
#undef MAP_NUMPAD_CHAR
		...
		case SDL_FINGERDOWN: {
			...
			ev.TouchInput.X = SDL_event.tfinger.x * Width;
			ev.TouchInput.Y = SDL_event.tfinger.y * Height;
			postEventFromUser(ev);
			++touchesCount;
			break;
		}
