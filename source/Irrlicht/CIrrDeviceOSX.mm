...
void CIrrDeviceMacOSX::handleInputEvent(const char *cStr)
{
	...
	irr::core::utf8ToWchar(cStr, ws, (lenOld + 1)*sizeof(wchar_t));
...
void CIrrDeviceMacOSX::postMouseEvent(void *event,irr::SEvent &ievent)
{
	...
	if (Window != NULL)
	{
		...
		ievent.MouseInput.X = (int)location.x;
		ievent.MouseInput.Y = DeviceHeight - (int)location.y;
		if (ievent.MouseInput.Y < 0 || ievent.MouseInput.X < 0 || ievent.MouseInput.Y > DeviceHeight || ievent.MouseInput.X > DeviceWidth)
			post = false;
	}
	else
	{
